...
export default class {
	...
	eventTunnel() {
		...
		this.ws = new WebSocket(`ws://${this.host}:${this.port}/event_tunnel?token=${this.token}`, this.port)

		this.ws.onopen = () => {
			var cnt = 1
			setInterval(() => {
				cnt++
				this.ws.send(JSON.stringify({ type: 'main.EventTest', message: cnt.toString(), from: this.clientID }))
			}, 1000)
		}
